...
AS
	INSERT INTO dbo.CUATROGDD2018.Usuarios (username, password)
	SELECT @usuario,HASHBYTES('SHA2_256', @contras)
	WHERE (NOT EXISTS (SELECT * FROM dbo.CUATROGDD2018.Usuarios WHERE username = @usuario))  
		  AND (NOT EXISTS (SELECT * FROM dbo.CUATROGDD2018.Personas WHERE email = @emailUsu))
	DECLARE @nuevoIDUsuario int
	SELECT @nuevoIDUsuario=SCOPE_IDENTITY() FROM CUATROGDD2018.Usuarios
	IF @nuevoIDUsuario is not null
		BEGIN
			INSERT INTO dbo.CUATROGDD2018.Usuario_X_Rol (id_usuario,id_rol)
			VALUES (@nuevoIDUsuario, @idRol)
			INSERT INTO dbo.CUATROGDD2018.Usuario_X_Hotel (id_usuario,id_hotel)
